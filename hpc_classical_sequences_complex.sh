#!/bin/bash
#SBATCH --job-name=cseq_complex
#SBATCH --output=logs/cseq_complex_%j.out
#SBATCH --error=logs/cseq_complex_%j.err
#
#SBATCH --partition=boost_usr_prod
#SBATCH --qos=boost_qos_lprod
#SBATCH --account=iscrc_qusala
#
# Classical Markov sequences + COMPLEX ansatz (same readout as quantum campaign):
#   isolates classical data vs TFIM while keeping RX-RY-RZ + continuous L_half/L_1.
#SBATCH --nodes=1
...
RHO="${RHO:-0.8}"
KS_TAG="${KS//,/-}"
OUTPUT_DIR="${OUTPUT_DIR:-results/classical_sequences/complex_ansatz_T${T}_d${D}_ks${KS_TAG}_L${LAYERS}_n${N_SEEDS}}"

echo "=== JOB ${SLURM_JOB_ID:-local} STARTED at $(date) on $(hostname) ==="
echo "Classical sequences + complex ansatz: T=$T d=$D ks=$KS layers=$LAYERS n_seeds=$N_SEEDS rho=$RHO"
