...
def make_qsa_state_qnode(T, d, k, c, layers=2, dev_name="default.qubit",
                         c_dtype=None, mpi=False, complex_gates=True, use_phases=None):
...
def make_qsa_projector_qnode(T, d, k, c, layers=2, dev_name="default.qubit", c_dtype=None,
                             complex_gates=True, use_phases=None):
    ...
    qml = _BK["qml"]
    if use_phases is None: use_phases = bool(complex_gates)
    cT, n, C1, C2, A, B, P, ntot = _layout(T, d, k)
    tri = triangular_state(T, cT); th = prep_angles(c)
    block = unitary_block if complex_gates else real_ortho_block
    dk = dict(wires=ntot)
    ...
    def circ(X, Y, Wp, Vp, phi):
        ...
        if use_phases: c1_phase(qml, phi, C1, cT, T)
        PREP()
        for i in range(T):
            cv = bits_msb(i, cT)
...
def self_check(T=3, d=4, k=2, layers=2, seed=0, tol=1e-6, check_projector=True,
    ...
    phi = _BK["jnp"].array(np.random.default_rng(seed + 9).uniform(0, 2 * np.pi, T)
                           if use_phases else np.zeros(T))
    block = unitary_block if complex_gates else real_ortho_block
    if use_phases is None: use_phases = bool(complex_gates)
    ...
    rep = classical_report(X, Y, Wm, Vm, k, beta,
                           phi=(np.array(phi) if use_phases else None))
    ok = abs(mu_ov - rep["mu"]) < tol
    gates = ("complex(RX,RY,RZ)+phases" if complex_gates else "real(RY)") + \
            ("" if use_phases else " NO-PHASES")
...
def main():
    ...
    print("--- REAL gates, NO phase correction (classical sequences) ---")
    self_check(T=3, d=4, k=1, complex_gates=False, use_phases=False)
    self_check(T=3, d=4, k=2, complex_gates=False, use_phases=False)
