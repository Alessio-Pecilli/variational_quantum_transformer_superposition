#!/bin/bash
#SBATCH --job-name=final_loss_LB
#SBATCH --output=logs/final_loss_LB_%j.out
#SBATCH --error=logs/final_loss_LB_%j.err
#
#SBATCH --partition=boost_usr_prod
#SBATCH --qos=boost_qos_lprod
#SBATCH --account=iscrc_qusala
#
# L_B loss campaign (classical-sequence ansatz, non-complex):
#   - k-QSA / k-CSA: train with L_B = -log F
#   - nl-CSA: train with L_half_uniform (Renyi)
#   - 4 plots: train/test loss vs k + L_B vs L_half comparison (mono + poly)
#SBATCH --nodes=1
...
KS_TAG="${KS//,/-}"
OUTPUT_DIR="${OUTPUT_DIR:-results/final_loss/LB_T${T}_d${D}_ks${KS_TAG}_L${QSA_LAYERS}_n${N_SEEDS}_test}"

echo "=== JOB ${SLURM_JOB_ID:-local} STARTED at $(date) on $(hostname) ==="
echo "L_B campaign: T=$T d=$D ks=$KS QSA_L=$QSA_LAYERS n_seeds=$N_SEEDS test_seed=$TEST_DATA_SEED"
...
EXTRA=(--isolate-jobs --loss-objective L_B)
if [[ "$NO_POLY" == "1" ]]; then
  EXTRA+=(--no-poly)
fi
...
srun --mpi=pmix_v3 --mem=0 --export=ALL --cpu-bind=cores "$VENV_PY" run_final_loss.py \
  ...
  --appendix-curves-ks "${APPENDIX_CURVES_KS:-3,5}" \
